...
struct connection
{
	...
	unsigned int submitted_counter;
...
static struct connection *connection_create(int sockfd)
{
	...
	rs = conn->efd = eventfd(0, EFD_NONBLOCK);
	if (rs < 0) {
		perror("error efd");
		exit(EXIT_FAILURE);
	}
...
static enum connection_state send_message(struct connection *conn)
{
	...
	const ssize_t max_buf_size = conn->send_len;
	ssize_t remaining_buf_size = max_buf_size - conn->message_size;
	ssize_t message_size = send(conn->sockfd, conn->send_buffer + conn->message_size, remaining_buf_size, 0);
...
static enum connection_state receive_request(struct connection *conn)
{
	...
	const ssize_t max_buf_size = BUFSIZ;
	ssize_t remaining_buf_size = max_buf_size - conn->recv_len;
	ssize_t num_bytes = recv(conn->sockfd, conn->recv_buffer + conn->recv_len, remaining_buf_size, 0);
...
static void put_header(struct connection *conn)
{
	...
	char header[BUFSIZ];
	int header_len = snprintf(header, BUFSIZ,
							  "HTTP/1.1 200 OK\r\n"
							  "Content-Type: text/html; charset=UTF-8\r\n"
							  "Content-Length: %ld\r\n"
							  "Connection: close\r\n"
							  "Date: Thu, 31 Dec 2009 20:55:48 +0000\r\n"
							  "Last-Modified: Mon, 31 Dec 2009 17:55:28 +0000\r\n"
							  "\r\n",
							  conn->total_size);
...
static void put_error(struct connection *conn)
{
	...
	const char *error_msg = "HTTP/1.1 404 Not Found\r\n"
							...
							"Content-Length: -\r\n"
							"Connection: close\r\n"
							"Date: Sun, 02 Jan 2010 09:26:16 +0000\r\n"
							"Last-Modified: Mon, 01 Jan 2010 12:43:16 +0000\r\n"
							"\r\n";
...
void handle_existing_connection(struct connection *conn, uint32_t events)
{
	if (events & EPOLLIN) {
		epollin_switch(conn);
...
int main(void)
{
	...
	while (1) {
		...
		if (rev.data.fd == listenfd)
			...
			handle_existing_connection(conn, rev.events);
	}
